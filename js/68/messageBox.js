...
const pcsMessageBox = (function () {
  ...
  return function (msg, modal, buttonArray, callback) {
    ...
    if (!buttonArray) {
      ...
      okButton.addEventListener('click', clear);
      buttonDiv.appendChild(okButton);
    }

    if (typeof buttonArray === 'function') {
      okButton.addEventListener('click', buttonArray);
    }

    else {

      buttonArray.forEach(buttonString => {

        const button = document.createElement('button');
        button.innerText = buttonString;
        if (callback) {
          button.addEventListener('click', () => callback(buttonString));
        }
